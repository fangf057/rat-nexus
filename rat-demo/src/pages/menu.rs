...
impl Component for Menu {
    ...
    fn render(&mut self, frame: &mut ratatui::Frame, cx: &mut Context<Self>) {
        ...
        if let Some(state_entity) = &self.state {
            cx.subscribe(state_entity);
            let app_state = state_entity.read(|s| s.clone()).unwrap_or_default();
    ...
    fn handle_event(&mut self, event: Event, _cx: &mut EventContext<Self>) -> Option<Action> {
        match event {
            Event::Key(key) => match key.code {
                ...
                KeyCode::Char('t') => {
                    if let Some(state_entity) = &self.state {
                        let _ = state_entity.update(|s| s.theme = s.theme.next());
                    }
                    None
                }
